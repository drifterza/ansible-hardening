...
set-env:
	...
	@if [ ! -z $${ERROR} ] && [ $${ERROR} -eq 1 ]; then \
		echo "$(BOLD)Example usage: \`NAME=nexus REGION=eu-west ENVIRONMENT=prod AWS_ACCESS_KEY_ID=YOURAWSKEY AWS_SECRET_ACCESS_KEY=YOURAWSSECRET ANSIBLE_SSH_USER=debian LINODE_API_KEY=SOMEGENERATEDKEY ROOT_PASSWORD=SOMEGENERATEDPASSWORD make plan\`$(RESET)"; \
		exit 1; \
	 fi
